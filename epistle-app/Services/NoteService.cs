...
namespace EpistleLibrary.Services
{
    public class NoteService
    {
        ...
        public static List<NoteModel> GetNotes(string username)
        {
            ...
            {
                foreach (var note in context.EpistleNotes.Where(x => x.User.Username == username))
                {
                    notes.Add(note);
                }
            }
        ...
        public static void AddNote(NoteModel note)
        {
            ...
            using (EpistleContext context = new())
            {
                try
                {
                    ...
                    context.EpistleNotes.Update(context.EpistleNotes.Where(x => x.Id == note.Id).FirstOrDefault());
                    context.SaveChanges();
                }
                catch (Exception)
                {
        ...
        public static NoteModel LoadNote(NoteModel note)
        {
            using (EpistleContext context = new())
            {
                note = context.EpistleNotes.FirstOrDefault(x => x.Id == note.Id);
            }
        ...
        public static List<NoteModel> LoadShelf(string title)
        {
            ...
            using (EpistleContext context = new())
            {
                foreach (var note in context.EpistleNotes.Where(x => x.Bookshelf.Title == title))
                {
                    notes.Add(note);
                }
            }
        ...
        public static void DeleteNote(NoteModel note)
        {
            using (EpistleContext context = new())
            {
                context.EpistleNotes.Remove(context.EpistleNotes.FirstOrDefault(x => x.Id == note.Id));
                context.SaveChanges();
            }
        }
        ...
        public static void AddToShelf(NoteModel note)
        {
            using (EpistleContext context = new())
            {
                try
                {
                    context.EpistleNotes.Where(x=>x.Id==note.Id).FirstOrDefault().Bookshelf = BookshelfService.GetShelf(note.Content);
                    context.SaveChanges();
                }
                catch(Exception)
                { }
